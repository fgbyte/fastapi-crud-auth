...
from api.deps import ALGORITHM, SECRET_KEY, db_dependency, bcrypt_context
...
def create_access_token(username: str, user_id: int, expires_delta: timedelta):
    ...
    # las variables pueden ser nones, por eso el type: ignore
...
async def login_for_access_token(from_data: Annotated[OAuth2PasswordRequestForm, Depends()], db: db_dependency):
    ...
    token = create_access_token(
        user.username, user.id, timedelta(minutes=20))  # type: ignore
